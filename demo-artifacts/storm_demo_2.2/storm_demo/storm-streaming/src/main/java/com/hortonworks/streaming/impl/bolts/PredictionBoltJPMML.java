...
/*		try
		{
//			System.out.println("Phoenix Driver PATH: " + phoenixDriverPath);
//			URLClassLoader loader = new URLClassLoader(new URL[] {new URL("file://"+phoenixDriverPath)}, null);
//
//			phoenixDriver = (Driver) loader.loadClass("org.apache.phoenix.jdbc.PhoenixDriver").newInstance();
	...
	public double[] enrichEvent(Tuple input)
	{
		...
		try
		{
			...
			conn = DriverManager.getConnection("jdbc:phoenix:"+topologyConfig.getProperty("hbase.zookeeper.server")+":2181:/hbase-unsecure");
	...
	private Evaluator instantiateJPMMLModel() {
		...
		try
		{
			pmmlFile = getPmmlModelFromHDFS(new Path(topologyConfig.getProperty("hdfs.url")+"/tmp/pmml/TruckDemoModel-pmml.xml"), conf);
		}
		catch(Exception e)
		{
	...
	* */
	...
	/*private static String getDisplayName(Field field, FieldName fieldName){
...
//
//			if(file.getPath().getName().contains("pmml")) {
//				continue;
//			}
//
...
//			String pmmlContent = writer.toString();
//		}
