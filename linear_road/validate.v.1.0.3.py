	...
	if carid not in cars:
		...
		if car[6] == pos:	
			...
			if car[7] >= 3:  # Meaning, _this_ is at least the fourth time, and 3 or greater was the last time
				...
				print "At least fourth time at this pos: " + str(car[6]) + " times"  #D
				###################################################################
				...
				if len(stopped[stopped_key]) > 1: # There is more than one car at this stopped_key 
		...
		##########################################################################
			...
			# accident_key = t[4]+"-"+t[6]+"-"+t[7]  # xway[4], dir[6], seg[7]  # The lane is not important
			temp_key = str(car[2])+"-"+str(car[4])+"-"+str(car[5])
			removed_accident_flag = False  #D
			if temp_key in accidents:
				accident = accidents[temp_key]
				if carid == accident[0] or carid == accident[1]: 
					print "Clearing accident: " + ','.join(map(str, accidents[temp_key]))  #D
					#accidents.pop(temp_key)  # 
					accidents[temp_key][3] = ptime  # No longer removing an accident, but setting a time greater than -1 to indicate clearance time 
			...
			#######################################################################################################################
				...
				for akey in accident_keys:
					if akey in accidents:	
						...
						car_min = ptime/60 + 1
						acc_noti_threshold_min = accident[2]/60 + 2
						acc_clear_min = accident[3]/60 + 1
						if ((car_min >= acc_noti_threshold_min and accident[3]==-1) or 
					    	    (car_min <= acc_clear_min and accident[3]!=-1)) :
							curr_toll = 0
							#############################################################	
							#############################################################	
