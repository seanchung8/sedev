...
#accidents = {}  # key:xway+lane+dir+seg; values: [carid1, carid2, time, clear_time]  # moving forward with only two cars can be part of an accident (we'll see if the mitsim data holds)
...
def t0(t):
	...
	if carid not in cars:
		...
		# Enter the car information with dummy values, -100 and 0 for prev. toll(s), that WON'T match anything coming in.
		## So, the only things that happen should be NEW SEGMENT, NUMV, LAV, ACCIDENT, and UPDATE 
		### Ensure speed from this faux "previous" pos isn't taken into account for LAV, and after this car will be updated with the current pos report info
		cars[carid] = [-100, -100, -100, -100, -100, -100, -100, 0, 0, 0, 0, 0] 
		print "Adding a NEW car: " + str(carid) + ": " +  ','.join(map(str,cars[carid]))  #D
	...
	car = cars[carid]
		# KEEP ANY PREVIOUS TOLL FROM A TOLL NOTIFICATION.  It still needs to be assessed on a new segment and a new pos report doesn't always need a new segment.  In fact, it will be very rare to find back-to-back new segments!
	if car == None:
		print "Fatal Error: no 'car' set!"
		sys.exit(1)
	else:
		last_toll = car[8]
		...
		if car[6] == pos:	
			...
			if car[7] >= 3:  # Meaning, _this_ is at least the fourth time, and 3 or greater was the last time
				...
				print "At least fourth time at pos " + str(car[6]) + " " + str(car[7]) + " times"  #D
				###################################################################
				...
				if len_stopped > 2:
					#print "Fatal Error: there should never be more than two stopped cars at a xway, lane, dir, seg, pos"
					#sys.exit(1)
					print "More than two stopped cars at this xway, lane, dir, seg, pos"
				if len_stopped > 1: # If there are two cars at this stopped_key 
					if accident_key not in accidents:  # If there isn't already an accident
						# BUT, if there's already an accident, and a third or fourth (or more) comes along and becomes part of the accident, and then that 3rd or 4th car moves, does that clear the accident or only the original two cars?  You would think it would only be the original two cars, right?  The others are just slow cars moving around the accident?
						print "Adding an accident..." #D	
						#accidents = {}  # key:xway+lane+dir+seg; values: [time, clear_time, carid1, carid2, ...]# moving forward with only two cars can be part of an accident (we'll see if the mitsim data holds)
						#accidents[accident_key] = [stopped_cars[0], stopped_cars[1], ptime, -1]
						accidents[accident_key] = [ptime, -1] 
					print_accidents()
					# Add stopped cars to accident key regardless of whether an accident has already been detected
		...
		##########################################################################
			...
			###########################################################
			# Just try the three or five keys
			# xway+lane+dir+seg+pos 
			# The lane, pos, and seg will be from the prior record, so don't search for the key, recreate it!
			# cars = {}  # key: carid; values: [time[0], speed[1], xway[2], lane[3], dir[4], seg[5], pos[6], xpos[7], ltoll[8], bal[9], bal1[10], bal2[11]]  # All the car values represent the _last_ or _previous_ car stats
			# stopped = {}  # key: xway+lane+dir+seg+pos; values [carid...]  # a list of carids
...
# cars = {}  # key: carid; values: [time[0], speed[1], xway[2], lane[3], dir[4], seg[5], pos[6], xpos[7], ltoll[8], bal[9], bal1[10], bal2[11]]  # All the car values represent the _last_ or _previous_ car stats
			...
			print_accidents()
			print "Potential accident carid: " + str(carid)
			if temp_key in accidents:
				print "Temp key for accident: " + temp_key
				accident = accidents[temp_key]
				# Delete accident from list if over one minute has past since it cleared (other accidents may still happen here, with the same cars?)
				# Or, are we saying only ONE accident can ever happen at any "key" during the simulation?  (This one, simpler, no code change)
				if accident[1] == -1 and (carid == accident[2] or carid == accident[3]):   # Clear if ONLY one of the original two cars moves
				#if accident[1] == -1 and carid in accident:   # Clear if ANY car (>= 2)
					print "Clearing accident: " + ','.join(map(str, accidents[temp_key]))  #D
					#accidents.pop(temp_key)  # 
					accident[1] = ptime  # No longer removing an accident, but setting a time greater than -1 to indicate clearance time 
			...
			if removed_accident_flag == True:  #D
				...
				# And, it was a wrong assumption, there can be more than two cars at an accident so, change the values to [time, clear_time, carid1, carid2,...]
					# Clear the accident
			...
			#######################################################################################################################
				...
				if last_min_key in segs:  # If we're just starting this key won't exist since it will be min = 0 
					...
					if numv > 50: 
						...
						print "TOLL FROM NUMV: " + str(curr_toll)
				######################################################
				...
				if total_speed_readings > 0:
					...
					if int(round(lav)) >= 40:  # If this doesn't hold true, we let the toll stand; don't really need the cast to int, but maybe it's safer
						curr_toll = 0  # look for any bugs with lav >= 40 having a toll -- so, the errors (toll even though lav>=40) occur from time 960 - 1703.  ???
