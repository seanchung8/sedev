...
QUERY_LIST[hive12]=list/pm20-1.1.csv
QUERY_LIST[hive13]=list/pm20-1.1.csv
QUERY_LIST[stinger3]=list/pm20-1.1.csv
#QUERY_LIST[stinger3]=list/pm50hive13.csv
TIMEOUT=600
MAXQUERIES=100
USER=hive
##### configuration end ######

function killAllYarnApplications() {
#should only use webhcat for this !!!!!
	yarn application -list | grep application | awk '{print $1}' | xargs -I XXX -P 10 yarn application -kill XXX
}
...
function runbench(){
  ...
  cd runner
  for SCALE in 30000 ; do
    for THREADS in 1 ; do
      for SYSTEM in stinger3 hive13 hive12; do
	#colmux >  report/$system_$threads_$scale_100_orc-$runid.colmux &
	#colmuxpid=????
	logfile="report/${runid}-${SYSTEM}_${THREADS}_${SCALE}_orc.txt"
echo "Started at " `date +%s` | tee ${logfile}	
./run_hiveserver.sh $SYSTEM $HOST ${PORT[$SYSTEM]} tpcds_bin_partitioned_orc_$SCALE $USER hive ${QUERY_LIST[$SYSTEM]} $THREADS $MAXQUERIES $TIMEOUT  | tee -a ${logfile} 
echo "Finished at " `date +%s` | tee -a ${logfile}

	#killAllYarnApplications
	#kill colmux
      done
    done
  done
...
function mkreport(){
	# Compute report
	cd runner
	for SCALE in 2 200 500; do
	for SYSTEM in hive12 stinger3; do
		if [ "$PLATFORM" == "HDP" ]; then
